...
if __name__=="__main__":
    ...
    rospy.init_node(multirotor_type +'_'+str(drone_number) + '_multirotor_keyboard_control') 
# rospy.init_node(multirotor_type +'_'+str(multirotor_num)+ '_multirotor_keyboard_control') # added '_'+str(multirotor_num)+ here
    
    if control_type == 'vel':
        multi_cmd_vel_flu_pub = [None]*multirotor_num
        multi_cmd_pub = [None]*multirotor_num
        for i in range(multirotor_num):
            multi_cmd_vel_flu_pub[i] = rospy.Publisher('/xtdrone/'+multirotor_type+'_'+str(drone_number)+'/cmd_vel_flu', Twist, queue_size=1)
            multi_cmd_pub[i] = rospy.Publisher('/xtdrone/'+multirotor_type+'_'+str(drone_number)+'/cmd',String,queue_size=3)
        ...
        multi_cmd_pub = [None]*multirotor_num
        for i in range(multirotor_num):
            multi_cmd_accel_flu_pub[i] = rospy.Publisher('/xtdrone/'+multirotor_type+'_'+str(drone_number)+'/cmd_accel_flu', Twist, queue_size=1)
            multi_cmd_pub[i] = rospy.Publisher('/xtdrone/'+multirotor_type+'_'+str(drone_number)+'/cmd',String,queue_size=1)
